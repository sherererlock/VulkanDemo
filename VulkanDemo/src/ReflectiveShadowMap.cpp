...
void ReflectiveShadowMap::CreatePipeline(PipelineCreateInfo& pipelineCreateInfo, VkGraphicsPipelineCreateInfo& creatInfo)
{
	...
	creatInfo.renderPass = renderpaass;
...
void ReflectiveShadowMap::CreateGBuffer()
{
	...
	CreateAttachment(&depth, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
}
...
void ReflectiveShadowMap::CreateAttachment(FrameBufferAttachment* attachment, VkFormat format, VkImageUsageFlagBits usage)
{
	...
	vulkanAPP->createImage(width, height, format, VK_IMAGE_TILING_OPTIMAL, usage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, attachment->image, attachment->mem, 1, VK_SAMPLE_COUNT_1_BIT);
