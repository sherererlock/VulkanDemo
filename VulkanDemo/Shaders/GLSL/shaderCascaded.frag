...
float PCF(vec3 coords, float filteringSize, uint index) {
	float bias = 0.0;
	if(shadowUbo.shadowIndex == 2)
		uniformDiskSamples(coords.xy);
	else if(shadowUbo.shadowIndex == 3)
...
float getShadow(vec3 coord, uint index)
{
	...
	switch(shadowUbo.shadowIndex)
	{
		...
		case 3:
			ivec2 texDim = textureSize(shadowMapSampler, 0).xy;
			float filteringSize = shadowUbo.filterSize / float(texDim.x);
